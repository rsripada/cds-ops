...
def task_run_core():
    ...
    if task_get_option("add-physical-copies-shelf-number-to-marc"):
        ...
        modified_rec_locs = run_sql("""SELECT id_bibrec, status, location, collection FROM crcITEM""")
        #Tagging of records
        if modified_rec_locs:
            ...
            for done, (recid, status, location, collection) in enumerate(modified_rec_locs):
                if not location or status not in [ CFG_BIBCIRCULATION_ITEM_STATUS_ON_SHELF, \
                ...
                for (_barcode, _loan_period, library_name, _library_id,
                    ...
                    MARC_RECS_STR += '<datafield tag="852" ind1=" " ind2=" "> \
                                      ...
                                      <subfield code="h">' + location +'</subfield> \
                                      </datafield>'
                ...
                if not done%1000:
                    ...
                    write_message("Processed %d out of %d modified record locations." % (done, total_modified_rec_locs))
                    MARC_RECS_STR = "<?xml version='1.0' encoding='UTF-8'?>\n<collection>"
                    task_sleep_now_if_required(can_stop_too=True)
