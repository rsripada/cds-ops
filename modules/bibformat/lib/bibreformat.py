...
def bibreformat_task(fmt, recids, without_fmt, process):
    ...
    @param sql: dictionary with pre-created sql queries for various cases (for
        selecting records). Some of these queries will be picked depending on
        the case
    @param sql_queries: a list of sql queries to be executed to select records
        to reformat.
    @param cds_query: a search query to be executed to select records to
        reformat
...
CFG_BIBFORMAT_UPDATE_FORMAT_FUNCTIONS = {'recjson': _update_recjson_format}
...
def iterate_over_new(recIDs, fmt):
    ...
    tot = len(recIDs)
    reformat_function = CFG_BIBFORMAT_UPDATE_FORMAT_FUNCTIONS.get(
        fmt, _update_format)
